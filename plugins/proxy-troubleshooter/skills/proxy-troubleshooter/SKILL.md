...
This skill should feel like a local agent with permission, not a support article. Do not send the user to click through a GUI for repetitive low-risk edits after they authorize Codex to perform a narrow fix.
...
```bash
python scripts/diagnose_proxy.py --target <hostname>
```
...
```powershell
scripts/diagnose-proxy.ps1 -Target <hostname> -SaveProfile -SaveRun
```
...
```bash
python scripts/apply_scoped_rule_fix.py --config <config.yaml> --domain <domain> --policy <policy>
python scripts/apply_scoped_rule_fix.py --config <config.yaml> --domain <domain> --policy <policy> --apply
```
...
```bash
python scripts/apply_scoped_rule_fix.py --config <config.yaml> --domain <domain> --move-existing
python scripts/apply_scoped_rule_fix.py --config <config.yaml> --domain <domain> --move-existing --apply
```
...
```bash
python scripts/record_feedback.py --run <state-dir>/runs/<run-file>.json --status fixed --solution-summary "Short confirmed fix"
python scripts/record_feedback.py --run <state-dir>/runs/<run-file>.json --status unchanged
python scripts/record_feedback.py --run <state-dir>/runs/<run-file>.json --status new-symptom --note "Short visible symptom"
```
